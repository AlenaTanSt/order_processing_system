...
OPS_TEST("Pipeline: no lost or duplicated orders (ids are unique and complete)") {
    ...
    for (std::uint64_t i = 0; i < N; ++i) {
        p.submit(MakeOrder(static_cast<int>(i)));
    }
...
OPS_TEST("Pipeline: submit increments accepted_count") {
    ...
    p.submit(MakeOrder(1));
    p.submit(MakeOrder(2));
    p.submit(MakeOrder(3));

    OPS_REQUIRE(p.metrics().accepted_count == 3);
    OPS_REQUIRE(p.delivered_orders().empty());
}

OPS_TEST("Pipeline: process_all delivers all orders and preserves timestamp invariants") {
    Pipeline p;
    ...
    OPS_REQUIRE(p.metrics().processed_count >= N);
    OPS_REQUIRE(p.metrics().delivered_count >= N);
}
...
OPS_TEST("Pipeline: metrics are cumulative across multiple process_all calls") {
    ...
    OPS_REQUIRE(m2.accepted_count == 40);
    OPS_REQUIRE(m2.processed_count >= 40);
    OPS_REQUIRE(m2.delivered_count >= 40);

    OPS_REQUIRE(m2.total_processing_time >= m1.total_processing_time);
    OPS_REQUIRE(p.delivered_orders().size() == 40);
}
